...
# Load RDS eset file
net_eset <- readRDS("data/ICGC/net_eset.rds")
...
nc <- (read.table("data/ICGC/nc_list.txt", header = T)$x)

table <- read.table("ICGC_NetBID2/SJAR/ICGC_NetBID2/input.exp", header = T)
nc_list_filt <- nc[nc %in% table$isoformId]

# Write the concatenated content to a new file
write.table(lnc_list_filt, "ICGC_NetBID2/SJAR/ICGC_NetBID2/nc_list.txt", row.names = F, quote = F, col.names = F)
