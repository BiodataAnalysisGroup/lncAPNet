...
sit_0 <- "LumA"
sit_1 <- "Basal"
Condition <- "paper_BRCA_Subtype_PAM50"
Comparison_name <- paste0(sit_1, ".Vs.", sit_0)

driver_output <- sprintf('%s/Driver_output', project_name)

source("../Adjusted_functions/NetBID.analysis.dir.create_nc.R")
...
# Merge network first
source("../Adjusted_functions/merge_TF_SIG_NC.network.R")
...
source("../Adjusted_functions/generate.masterTable_nc.R")
analysis.par$final_ms_tab <- generate.masterTable_lnc(use_comp=all_comp,
                                                     DE=analysis.par$DE,
                                                     DA=analysis.par$DA,
...
write.csv(activity, file = "../../../data/TCGA_Breast/activity_matrix.csv")
write.csv(phe_info, file = "../../../data/TCGA_Breast/metadata.csv")
