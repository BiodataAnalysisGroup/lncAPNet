...
counts_table <- read.csv("data/ICGC/count_matrix.csv", row.names = 1)
metadata <- read.csv("data/ICGC/metadata.csv", row.names = 1)
...
write.table(list_nc$hgnc_symbol, file = "data/ICGC/nc_list.txt", row.names = F, quote = F)
...
saveRDS(eset, "data/ICGC/net_eset.rds")
