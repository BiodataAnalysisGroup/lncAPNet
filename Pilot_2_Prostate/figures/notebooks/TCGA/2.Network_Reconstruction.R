...
features <- fData(net_eset)
lnc <- filter(features, gene_biotype != "protein_coding")
lnc <- lnc$hgnc_symbol

table <- read.table("Prostate_NetBID2/SJAR/Prostate_NetBID2/input.exp", header = T)
lnc_list_filt <- lnc[lnc %in% table$isoformId]

# Write the concatenated content to a new file
write.table(lnc_list_filt, "Prostate_NetBID2/SJAR/Prostate_NetBID2/lnc_list.txt", row.names = F, quote = F, col.names = F)
